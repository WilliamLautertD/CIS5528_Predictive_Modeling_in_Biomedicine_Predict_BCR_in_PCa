...
# =========================
ROOT_DIR = "images"
OUT_DIR = "outputs_bcr_mri_cv_reduced_pooling_avgmax_classweight_gpu"
...
#BCR_POS = ['1041', '1090', '1100', '1122', '1130', '1137', '1169', '1208', '1219', '1258']
#BCR_NEG = ['1036', '1060', '1129', '1140', '1179', '1186', '1205', '1264', '1284', '1303']
...
CASE_LABELS = {cid: 1 for cid in TRAIN_BCR_POS}
CASE_LABELS.update({cid: 0 for cid in TRAIN_BCR_NEG})
...
# 4) FIND CASE FILES
# =========================
def find_case_files(root_dir, allowed_case_ids):
    items = []
...
class MRIEncoder3DThreeStemSharedTrunk(nn.Module):
    """
    3 modality-specific stems:
      - T2W stem
      - ADC stem
      - HBV stem

    Then concatenate along channel dimension and continue with a shared trunk.
    ...
    def __init__(self, embed_dim=64, dropout=0.3):
        ...
        # Small modality-specific stems
        # Each modality is 1 channel -> 8 channels
        self.stem_t2w = ConvBlock(1, 8)
        self.stem_adc = ConvBlock(1, 8)
        self.stem_hbv = ConvBlock(1, 8)

        # Keep the same reduced pooling idea
        self.pool1 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))

        # After concatenation: 8 + 8 + 8 = 24 channels
        ...
        # Same final pooling strategy that worked best so far
    ...
    def forward(self, x):
        ...
        f_t2w = self.stem_t2w(x_t2w)   # (B, 8, D, H, W)
        f_adc = self.stem_adc(x_adc)   # (B, 8, D, H, W)
        f_hbv = self.stem_hbv(x_hbv)   # (B, 8, D, H, W)

        # Optional reduced pooling after each stem
        f_t2w = self.pool1(f_t2w)
        f_adc = self.pool1(f_adc)
        f_hbv = self.pool1(f_hbv)

        # Concatenate modality features
        x = torch.cat([f_t2w, f_adc, f_hbv], dim=1)  # (B, 24, D, H, W)
        ...
        x_avg = self.gap(x).flatten(1)   # (B, 64)
        x_max = self.gmp(x).flatten(1)   # (B, 64)
...
def extract_embeddings(model, loader, out_dir, fold_name="all"):
    ...
    np.save(os.path.join(out_dir, f"{fold_name}_mri_bcr_embeddings.npy"), all_z)
    pd.DataFrame({
        "case_id": all_ids,
        "label": all_labels,
    }).to_csv(os.path.join(out_dir, f"{fold_name}_mri_bcr_embedding_ids.csv"), index=False)

    with open(os.path.join(out_dir, f"{fold_name}_mri_bcr_embedding_meta.json"), "w") as f:
        json.dump({
...
def print_feature_map_shapes(model, input_shape=(1, 3, 48, 96, 96)):
    ...
    with torch.no_grad():
        x = model.encoder.enc1(x)
        print(f"enc1:     {tuple(x.shape)}")
        x = model.encoder.pool1(x)
        print(f"pool1:    {tuple(x.shape)}")
...
def main():
    ...
    case_ids = TRAIN_BCR_POS + TRAIN_BCR_NEG
    items = find_case_files(ROOT_DIR, case_ids)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.arange(len(items)), labels), start=1):
        ...
        train_ds = BCRMriDataset(train_items, target_shape=TARGET_SHAPE, augment=True)
        val_ds = BCRMriDataset(val_items, target_shape=TARGET_SHAPE, augment=False)
        ...
        fold_path = os.path.join(OUT_DIR, f"best_bcr_mri_fold{fold}.pt")
